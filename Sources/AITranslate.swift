...
struct AITranslate: AsyncParsableCommand {
  ...
  mutating func run() async throws {
    do {
      ...
      if swiftOnly {
        try generateSwiftEnumsFile(from: dict)
  ...
  func generateSwiftEnumsFile(from dict: StringsDict) throws {
    let fileName = inputFile.deletingPathExtension().lastPathComponent
    let swiftFileName = "\(fileName).swift"
    let outputURL = inputFile.deletingLastPathComponent().appendingPathComponent(swiftFileName)
